...
import sPrefsFile
import sFileDialog
from sStyleDialog import sStyleDialog, sSimpleStyleDialog
from sPreferences import sPreferences

class sPrefsBook(wx.Panel):
...
class PrefsPanel(wx.ScrolledWindow):
    def __init__(self, parent, id, name, NumberOfItems):
        ...
        self.sframe = self.grandparent.parent
...
class BookmarksPanel(PrefsPanel):
    ...
    def OnbtnBStyle(self, event):
        d = sSimpleStyleDialog(self, -1, ("Style: Bookmarks"), self.grandparent.prefs.bookmarksstyle, self.grandparent.prefs.bookmarksstyle)
...
class DocumentPanel(PrefsPanel):
    ...
    def Ontxtdoclonglinecol(self, event):
        ...
        try:
            ...
            if not x == "-":
                sScrolledMessageDialog.ShowMessage(self, ("Whatever you are trying to do... Stop it.\nIt won't work.  Actual Numbers only please.\nSeer will reset to the value last loaded."), "Value Error")
                self.txtdoclonglinecol.SetValue(str(self.grandparent.prefs.doclonglinecol))
                return
            y = 0
        except TypeError:
            sScrolledMessageDialog.ShowMessage(self, ("\"" + y + "\" is not an integer (a number, eg 2, 4, 568)\nSeer will reset to the value last loaded."), "Type Error")
            self.txtdoclonglinecol.SetValue(str(self.grandparent.prefs.doclonglinecol))
            return
        if y > 1024:
            sScrolledMessageDialog.ShowMessage(self, ("Seer does not recommend going this high.  You can of course..."), "Hmmmm")
    ...
    def OnbtnStyle(self, event):
        d = sStyleDialog(self, -1, ("Edit Styles"))
        d.ShowModal()
        if d.ClickedOk():
...
class DocumentationPanel(PrefsPanel):
    ...
    def OnbtnBrowse(self, event):
        dlg = sFileDialog.FileDialog(self.grandparent.parent, "Select A Browser", self.wildcard)
    ...
    def OnbtnBrowseDoc(self, event):
        ...
        dlg = sFileDialog.FileDialog(self.grandparent.parent, "Location of Documentation", self.wildcard)
        if dlg.ShowModal() == wx.ID_OK:
            result = dlg.GetPath()
        dlg.Destroy()
...
class FileDialogPanel(PrefsPanel):
    ...
    def OnEditWildcard(self, event):
        import sFileDialogPrefs
        d = sFileDialogPrefs.sFilePrefsDialog(self)
        answer = d.ShowModal()
        newwc = d.GetWildcard()
    ...
    def OnReplaceTable(self, event):
        import sFileDialogPrefs
        d = sFileDialogPrefs.sReplaceTableDialog(self)
        answer = d.ShowModal()
...
class FileTypesPanel(PrefsPanel):
    ...
    def __init__(self, parent, id, name):
        ...
        self.filetype = self.sframe.txtDocument.filetype
...
class GeneralPanel(PrefsPanel):
    ...
    def OnbtnExportPrefs(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            sZip.AddDirectoryToZipFile(self.sframe.preferencesdirectory,'',zf)
            zf.close()            
        dlg.Destroy()
...
class PromptPanel(PrefsPanel):
    ...
    def OnbtnStyle(self, event):
        d = sStyleDialog(self, -1, ("Edit Prompt Styles"), True)
        d.ShowModal()
        if d.ClickedOk():
...
class SourceBrowserPanel(PrefsPanel):
    ...
    def OnbtnCStyle(self, event):
        d = sSimpleStyleDialog(self, -1, ("Style: Source Browser"), self.grandparent.prefs.sourcebrowserstyle, self.grandparent.prefs.sourcebrowserstyle)
...
class sPrefsDialog(wx.Dialog):
    ...
    def __init__(self, parent, id, title):
        ...
        self.prefs = sPreferences(parent.PLATFORM_IS_WIN, parent.programdirectory)
        self.prefs.Copy(parent.prefs)
        self.oldprefs = sPreferences(parent.PLATFORM_IS_WIN, parent.programdirectory)
        self.oldprefs.Copy(parent.prefs)

        self.lbPrefs = sPrefsBook(self, self.ID_LB_PREFS, (600, 400))
...
(You still need to click update and/or save)\nAre you sure you want to do this?", "Reset Panel Preferences", wx.YES_NO | wx.ICON_QUESTION)
    ...
    def OnbtnSave(self, event):
        ...
        if not os.path.exists(self.parent.preferencesdirectory):
            sScrolledMessageDialog.ShowMessage(self, "Your userpreferencesdirectory '" + self.parent.preferencesdirectory + "' does not exist.", "Seer Error")
            return
        preffile = self.parent.prefsfile
        try:
            sPrefsFile.WritePreferences(self.GetPreferences(), preffile)
        except IOError:
