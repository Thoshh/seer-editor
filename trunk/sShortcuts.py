...
def SetSTCShortcuts(stc, Shortcuts, useDefault = 0):
    ...
    #Note:  If stc.CmdKeyAssign is used, then Seer will never process
...
def RunShortcuts(frame, event, stc, SplitView):
    ...
    if stc.IsAPrompt:
        if (frame.STCCOMMANDLIST[sstc] == wx.stc.STC_CMD_NEWLINE) or \
        (frame.STCCOMMANDLIST[sstc] == wx.stc.STC_CMD_CHARLEFT) or \
        (frame.STCCOMMANDLIST[sstc] == wx.stc.STC_CMD_CHARRIGHT) or \
        (frame.STCCOMMANDLIST[sstc] == wx.stc.STC_CMD_LINEUP) or \
        (frame.STCCOMMANDLIST[sstc] == wx.stc.STC_CMD_LINEDOWN) or \
        (frame.STCCOMMANDLIST[sstc] == wx.stc.STC_CMD_DELETEBACK) or \
        (frame.STCCOMMANDLIST[sstc] == wx.stc.STC_CMD_HOME):
    ...
    if sstc > -1 and (not seer > -1) and (not sscript > -1):
        try:
            if frame.STCCOMMANDLIST[sstc] == wx.stc.STC_CMD_PASTE:
                ...
                stc.CmdKeyExecute(frame.STCCOMMANDLIST[sstc])
        except:
            return -1
        return frame.STCCOMMANDLIST[sstc]
    if SplitView:
