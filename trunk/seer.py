...
class sFrame(wx.Frame):
    def __init__(self, parent, id, title):
        ...
        self.Bind(wx.EVT_MENU,  self.OnSyntaxHighlightingRuby, id=self.ID_HIGHLIGHT_RUBY)
        self.Bind(wx.EVT_MENU,  self.OnSyntaxHighlightingFortran, id=self.ID_HIGHLIGHT_FORTRAN)
    ...
    def checkiffileisRuby(self, filename):
        return self.refiletypeisrb.search(filename) is not None
        
    def checkiffileisFortran(self, filename):
        return self.refiletypeisfor.search(filename) is not None
    ...
    def InitializeConstants(self):
        ...
        self.ID_HIGHLIGHT_RUBY = 590
        self.ID_HIGHLIGHT_FORTRAN = 591
    ...
    def OnSaveAs(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            if not self.prefs.doconlyusedefaultsyntaxhighlighting:
                ...
                elif self.checkiffileisRuby(self.txtDocument.filename):
                    #Ruby
                    self.txtDocument.filetype = 4
                    self.highlightmenu.Check(self.ID_HIGHLIGHT_RUBY, True)
                elif self.checkiffileisFortran(self.txtDocument.filename):
                    #Fortran
                    self.txtDocument.filetype = 3
                    self.highlightmenu.Check(self.ID_HIGHLIGHT_FORTRAN, True)
                else:
                    #Default
                    pass
                self.txtDocument.SetupPrefsDocument()
        ...
        try:
            ...
            if not self.prefs.doconlyusedefaultsyntaxhighlighting:
                if self.checkiffileisPython(filename):
                    ...
                    self.txtDocument.filetype = 3
                    self.highlightmenu.Check(self.ID_HIGHLIGHT_PLAIN_TEXT, True)
                elif self.checkiffileisRuby(filename):
                    #Ruby
                    self.txtDocument.filetype = 4
                    self.highlightmenu.Check(self.ID_HIGHLIGHT_RUBY, True)
                elif self.checkiffileisFortran(filename):
                    #Fortran
                    self.txtDocument.filetype = 5
                    self.highlightmenu.Check(self.ID_HIGHLIGHT_FORTRAN, True)
                else:
    ...
    def setDocumentTo(self, number, ignoreold = 0):
        ...
        if self.txtDocument.filetype == 3:
            self.highlightmenu.Check(self.ID_HIGHLIGHT_PLAIN_TEXT, True)
        if self.txtDocument.filetype == 4:
            self.highlightmenu.Check(self.ID_HIGHLIGHT_RUBY, True)
        if self.txtDocument.filetype == 5:
            self.highlightmenu.Check(self.ID_HIGHLIGHT_FORTRAN, True)
    ...
    def setupfiletypeextensions(self):
        ...
        self.refiletypeisrb = re.compile(self.getfileextensionstring(4), re.IGNORECASE)
        self.refiletypeisfor = re.compile(self.getfileextensionstring(5), re.IGNORECASE)
    ...
    def CreateMenus(self):
        ...
        self.highlightmenu.AppendRadioItem(self.ID_HIGHLIGHT_RUBY, "Ruby")
        self.highlightmenu.AppendRadioItem(self.ID_HIGHLIGHT_FORTRAN, "Fortran")
        self.highlightmenu.Check(self.ID_HIGHLIGHT_PYTHON, True)
