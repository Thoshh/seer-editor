...
class sFileDialog(wx.Dialog):
    ...
    def OnbtnBookmarks(self, event):
        ...
        if os.path.exists(bookfile):
            try:
                ...
                sScrolledMessageDialog.ShowMessage(self, ("Your bookmarks file is a tad messed up.\n"), "Error")
                return
        else:
            f = file(bookfile, 'wb')
            f.write('>Bookmarks\n')
        ...
        if os.path.exists(filename) and not os.path.isdir(filename):
            ...
            if self.parent.Ask(msg, "Delete File?"):
                try:
                    ...
                    sScrolledMessageDialog.ShowMessage(self, 'Error Removing "' + filename + '".', "Error")
    ...
    def OnbtnHome(self, event):
        try:
            ...
            sScrolledMessageDialog.ShowMessage(self, 'Error Setting Current Directory to Home', "sPython Error")
...
class sWxFileDialog(wx.FileDialog):
...
def FileDialog(parent, title, wildcard, point=(0, 0), size=wx.DefaultSize, IsASaveDialog=0, MultipleSelection=0, ShowRecentFiles=0):
        if parent.prefs.usewxfiledialog:
            if IsASaveDialog:
                return sWxFileDialog(parent, title, "", "", wildcard, wx.SAVE|wx.HIDE_READONLY|wx.OVERWRITE_PROMPT)
            else:
                return sWxFileDialog(parent, title, "", "", wildcard, wx.OPEN|wx.HIDE_READONLY|wx.MULTIPLE)
        else:
            return sFileDialog(parent, title, wildcard, point, size, IsASaveDialog, MultipleSelection, ShowRecentFiles)
