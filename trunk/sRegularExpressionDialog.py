...
class sRegularExpressionDialog(wx.Frame):
    def __init__(self, parent, id, title, prompthasfocus = 0, infiles = 0):
        ...
        self.txtRE = sRETextCtrl(self, -1, "", wx.DefaultPosition, wx.Size(500, -1))
    ...
    def OnLoad(self, event):
        dlg = sFileDialog.FileDialog(self.filedialogparent, "Load Regular Expression From", wildcard)
        if self.defaultdirectory:
            try:
                dlg.SetDirectory(self.defaultdirectory)
            except:
                sScrolledMessageDialog.ShowMessage(self, ("Error Setting Default Directory To: " + self.defaultdirectory), "Seer Error")
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try:
                ...
                sScrolledMessageDialog.ShowMessage(self, ("Error Reading From: " +  refile), "Seer Error")
                text = ""
            if (text.find('\n') > -1) or (text.find('\r') > -1):
                sScrolledMessageDialog.ShowMessage(self, ("Error Reading From: " +  refile), "Seer Error")
                text = ""
    ...
    def OnSave(self, event):
        dlg = sFileDialog.FileDialog(self.filedialogparent, "Save Regular Expression As", wildcard, IsASaveDialog=True)
        if self.defaultdirectory:
            try:
                dlg.SetDirectory(self.defaultdirectory)
            except:
        ...
        if dlg.ShowModal() == wx.ID_OK:
            ...
            try:
                ...
                sScrolledMessageDialog.ShowMessage(self, ("Error Writing To: " +  refile), "Seer Error")
                return
            if self.enablefeedback:
                sScrolledMessageDialog.ShowMessage(self, ("Successfully Saved: " + refile), "Save Success")
            dlg.Destroy()
        self.Raise()
