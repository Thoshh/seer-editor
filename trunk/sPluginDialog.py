...
class sUpdatePluginListDialog(wx.Dialog):
    ...
    def RunInThread(self):
        ...
        targetfile = self.parent.ancestor.pluginsdirectory + "/seer.plugin.list.dat"
        try:
...
If you proceed, Seer will use the copy
...
class sSelectPluginsDialog(wx.Dialog):
    def __init__(self, parent):
        wx.Dialog.__init__(self, parent, -1, "Select Seer Plugin", wx.DefaultPosition, wx.Size(600, 400), wx.DEFAULT_DIALOG_STYLE)
        ...
        #Local For Test
        self.pluginlistfile = parent.pluginsdirectory + "/seer.plugin.list.dat"
        if not os.path.exists(self.pluginlistfile):
            self.pluginlistfile = parent.programdirectory + "/seer.plugin.list.dat"
...
<br></html></body>''' % (txt, self.PluginDataArrayForLST[sel][0], self.PluginDataArrayForLST[sel][1], self.PluginDataArrayForLST[sel][2])
    ...
    def ReadPluginList(self):
        try:
            ...
            if not self.Plugins:
                sScrolledMessageDialog.ShowMessage(self, 'Corrupt Plugin List, Trying Default', 'Error')
                self.pluginlistfile = self.parent.programdirectory + "/seer.plugin.list.dat"
                f = file(self.pluginlistfile, 'r')
                text = f.read()
                f.close()
            ...
            sScrolledMessageDialog.ShowMessage(self, 'Error Reading "' + self.pluginlistfile + '".', 'Error Reading Plugin List')
...
class sPluginInstallLocationPage(wx.wizard.WizardPageSimple):
    def __init__(self, parent):
...
class sPluginInstallSelectPage(wx.wizard.WizardPageSimple):
    def __init__(self, parent):
        wx.wizard.WizardPageSimple.__init__(self, parent)
        self.parent = parent
    ...
    def OnAdd(self, event):
        if self.parent.Location:
            #SourceForge
            d = sSelectPluginsDialog(self.parent.ancestor)
            answer = d.ShowModal()
            self.Raise()
            self.SetFocus()
            if answer == wx.ID_OK:
            ...
            #Local
            dlg = sFileDialog.FileDialog(self.parent.ancestor, "Select Seer Plugin(s)", self.parent.wildcard, MultipleSelection=True)
    ...
    def Run(self):
        if self.parent.Location:
            ...
            if answer == wx.YES:
                e = sUpdatePluginListDialog(self)
                e.ShowModal()
                e.Destroy()
...
class sPluginInstallDownloadPage(wx.wizard.WizardPageSimple):
    def __init__(self, parent):
        wx.wizard.WizardPageSimple.__init__(self, parent)
    ...
    def UpdateUI(self, event):
        ...
        if self.errors:
            ...
            sScrolledMessageDialog.ShowMessage(self.parent, 'Error Downloading File "' + error[0] + '".\n\n' + error[1], "Error", wx.DefaultPosition, wx.Size(550,300))

class sPluginInstallIndexPage(wx.wizard.WizardPageSimple):
    def __init__(self, parent):
        wx.wizard.WizardPageSimple.__init__(self, parent)
...
class sPluginInstallInstallPage(wx.wizard.WizardPageSimple):
    def __init__(self, parent):
        wx.wizard.WizardPageSimple.__init__(self, parent)
    ...
    def Index(self, label, unzippedfiles, installmethod):
        ...
        if installmethod == 0:
            ...
            try:
                ...
                sScrolledMessageDialog.ShowMessage(self.parent, 'Error Adding Plugin "' + pluginname +
            ...
            if not indexfile:
                sScrolledMessageDialog.ShowMessage(self.parent, 'Error Installing "' + label + '"', 'Install Index Error')
                return
    ...
    def Install(self, filename, label, unzippedfiles):
        ...
        if not pluginfile:
            sScrolledMessageDialog.ShowMessage(self.parent, 'Error Installing "' + label + '"', 'Install Error')
        else:
            #Do the Install
    ...
    def Run(self):
        ...
        for label in self.parent.pluginlabels:
            ...
            try:
                ...
                sScrolledMessageDialog.ShowMessage(self.parent, ("Error removing temporary directory for: " + label), "Post Install Error")
                return

        self.stxtDone.SetForegroundColour(self.GetForegroundColour())

class sPluginInstallWizard(wx.wizard.Wizard):
    def __init__(self, parent, title, bitmap):
        ...
        self.wildcard = "Seer Plugin Archive (*.zip)|*.zip"
        ...
        try:
            ...
            sScrolledMessageDialog.ShowMessage(parent, "Error Getting Temporary Directory.", "Install Error")
            return

        self.LocationPage = sPluginInstallLocationPage(self)
        self.SelectPage = sPluginInstallSelectPage(self)
        self.DownloadPage = sPluginInstallDownloadPage(self)
        self.IndexPage = sPluginInstallIndexPage(self)
        self.InstallPage = sPluginInstallInstallPage(self)
...
class sPluginUnInstallDataPage(wx.wizard.WizardPageSimple):
    def __init__(self, parent):
        wx.wizard.WizardPageSimple.__init__(self, parent)
...
class sPluginUnInstallUnInstallPage(wx.wizard.WizardPageSimple):
    def __init__(self, parent):
        wx.wizard.WizardPageSimple.__init__(self, parent)
    ...
    def RemoveDatFiles(self, plugin):
        ...
        try:
            ...
            sScrolledMessageDialog.ShowMessage(self.parent, "Error Removing Plugin Preference Files", "Uninstall Error")
    ...
    def Run(self):
        ...
        for idx in IndexList:
            try:
                ...
                sScrolledMessageDialog.ShowMessage(self.parent, ('Error Removing Plugins From Index: "' + idx + '".'), "Uninstall Error")
            x = x + 1
    ...
    def UnInstall(self, plugin):
        ...
        try:
            ...
            sScrolledMessageDialog.ShowMessage(self.parent, ("Error Removing Plugin File: " + pluginfile), "Uninstall Error")

class sPluginUnInstallWizard(wx.wizard.Wizard):
    def __init__(self, parent, title, bitmap):
        wx.wizard.Wizard.__init__(self, parent, 303, title, bitmap)
        ...
        self.SelectPage = sPluginUnInstallSelectPage(self)
        self.DataPage = sPluginUnInstallDataPage(self)
        self.UnInstallPage = sPluginUnInstallUnInstallPage(self)
...
class sEditIndexDialog(wx.Dialog):
    ...
    def OnbtnAdd(self, event):
        ...
        try:
            ...
            sScrolledMessageDialog.ShowMessage(self, 'Plugin "' + tselection + '" has already been added.', "Already Added")
        except:
            tsel = self.boxNotInIndex.GetSelection()
            if tsel == -1:
                sScrolledMessageDialog.ShowMessage(self, "Nothing Selected to Add", "Mistake")
                return
    ...
    def OnbtnDelete(self, event):
        ...
        if indexname == 'default.idx':
            sScrolledMessageDialog.ShowMessage(self, 'You Cannot Delete the Default Index', 'Error')
            return
        answer = wx.MessageBox('Delete "%s"?' % (indexname), "Seer", wx.YES_NO | wx.ICON_QUESTION)
        if answer == wx.YES:
            indexfile = os.path.join(self.parent.pluginsbasepreferencesdir, indexname)
            if not os.path.exists(indexfile):
                sScrolledMessageDialog.ShowMessage(self, '"%s" does not exist.' % (indexfile), 'Error')
                return
            try:
                os.remove(indexfile)
            except:
                sScrolledMessageDialog.ShowMessage(self, 'Error Removing "%s".' % (indexfile), 'Error')
            ...
            if self.parent.prefs.enablefeedback:
                sScrolledMessageDialog.ShowMessage(self, 'Successfully Removed "%s".' % (indexfile), 'Deleted Index')
    ...
    def OnbtnNew(self, event):
        ...
        if answer == wx.ID_OK:
            ...
            try:
                ...
                sScrolledMessageDialog.ShowMessage(self, 'Error Creating "%s".' % (indexfile), 'Error')
                return
    ...
    def OnbtnRemove(self, event):
        sel = self.boxInIndex.GetSelection()
        if sel == -1:
            sScrolledMessageDialog.ShowMessage(self, "Nothing Selected to Remove", "Mistake")
            return
    ...
    def OnbtnSaveAs(self, event):
        ...
        if answer == wx.ID_OK:
            ...
            try:
                ...
                sScrolledMessageDialog.ShowMessage(self, 'Error Saving "%s" As "%s".' % (idx, indexname), 'Error')
                return
    ...
    def OnOpen(self, event):
        ...
        try:
            ...
            sScrolledMessageDialog.ShowMessage(self.parent, ("Error Reading Index File: " + self.indexname), "Open Error")
