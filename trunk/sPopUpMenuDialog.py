...
def GetPopUpMenuLabels(filename, frame):
    ...
    rePopUpMenu = re.compile(r'^\s*?sFrame\.AddPluginPopUpMenuFunction\(.*\)', re.MULTILINE)
...
class sPopUpMenuDialog(wx.Dialog):
    ...
    def OnbtnRemove(self, event):
        sel = self.popupmenu.GetSelection()
        if not sel:
            sScrolledMessageDialog.ShowMessage(self, "You cannot remove the root item.", "Mistake")
            return
        if sel == -1:
