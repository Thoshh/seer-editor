...
class sText(sSTC.sStyledTextControl):
            ...
            if (modify != self.modified) or (event is None):
                ...
                if self.modified:
                    ...
                    if not self.filename:
                        ...
                        self.grandparent.SetTitle("Seer - Untitled " + str(self.untitlednumber) + ' [Modified]')
                        ...
                        self.grandparent.SetTitle("Seer - " + self.filename + " [Modified]")
                else:
                    if self.IsActive:
                    ...
                    if not self.filename:
                        self.notebookparent.SetPageText(self.targetPosition, "Untitled " + str(self.untitlednumber))
                        self.grandparent.SetTitle("Seer - Untitled " + str(self.untitlednumber))
                    else:
                        self.notebookparent.SetPageText(self.targetPosition, os.path.basename(self.filename))
                        self.grandparent.SetTitle("Seer - " + self.filename)
    ...
    def SetupLineNumbersMargin(self):
        if self.grandparent.prefs.docshowlinenumbers:
            ...
            textwidth = self.TextWidth(wx.stc.STC_STYLE_LINENUMBER, sEncoding.EncodeText(self.grandparent, lstring, self.encoding))
    ...
    def SetupPrefsDocument(self, notmdiupdate = 1):
        ...
        self.SetKeyWords(0, sKeywords.GetKeyWords(self.filetype))

        self.SetLexer(sKeywords.GetLexer(self.filetype))
        ...
        if self.grandparent.prefs.docusestyles:
            ...
            if (self.grandparent.prefs.docusestyles < 2) or (not self.filetype == 4):
                ...
                sKeywords.SetSTCStyles(self.grandparent, self, self.filetype)
