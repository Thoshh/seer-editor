...
def WriteBranch(tree, branch, filehandle, tablevel):
    ...
    filehandle.write(y)
    if isfolder:
        ccount = tree.GetChilsenCount(branch, 0)
        if ccount > 0:
...
class sBookmarksDialog(sTreeDialog):

    def __init__(self, parent, bookmarksfile):

        sTreeDialog.__init__(self, parent, 'Edit Bookmarks', 'Bookmarks', bookmarksfile, parent.prefs.bookmarksstyle, \
    ...
    def OnbtnAdd(self, event):
        ...
        if self.datatree.GetItemText(sel)[0] == '>':
                ...
                if s == "Type It In":
                    ...
                    if self.parent.ddirectory:
                        try:
                            ...
                            sScrolledMessageDialog.ShowMessage(self.parent, ("Error Setting Default Directory To: " + self.parent.ddirectory), "Seer Error")
                    if d.ShowModal() == wx.ID_OK:
                        currentItem = self.datatree.AppendItem(sel, d.GetPath().replace('\\', '/'))
                    d.Destroy()
                else:
                    dlg = sFileDialog.FileDialog(self.parent, "Select File", self.wildcard)
                    if self.parent.ddirectory:
                        try:
            ...
            sScrolledMessageDialog.ShowMessage(self, "You can only add a bookmark to a folder.\nSelect either \"Bookmarks\", or a folder.", "Bad Bookmark Folder")
